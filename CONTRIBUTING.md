# Contributing to Milpa HTTP

Thanks for your interest in contributing! Milpa HTTP is the web tier of the Milpa
framework — routing, templating, and the web lifecycle events. It builds on
`milpa/core`: attributes, interfaces, events, and route definitions for the HTTP
layer, with no product coupling.
...
- **Respect the tier boundary.** Milpa HTTP depends on `milpa/core`, never the
  reverse. Do not introduce a dependency on Doctrine, a concrete container, or any
  product/plugin code, and do not push HTTP concerns down into the core.
